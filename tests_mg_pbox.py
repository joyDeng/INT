...
def two_sphere_multi_energy_sensor(num_neutrons, param_dict, seed):
    ...
    sig_t_in = param_dict["sig_t"]
    # Support both the old 1-parameter form (scalar) and the new 3-parameter form (length-3 iterable)
    if isinstance(sig_t_in, (list, tuple, np.ndarray)):
        if len(sig_t_in) != 3:
            raise ValueError("param_dict['sig_t'] must have length 3 for multi-group sensitivity.")
        sig_t_inner = [FloatD(sig_t_in[0]), FloatD(sig_t_in[1]), FloatD(sig_t_in[2])]
    else:
        sig_t_inner = [sig_t_in, FloatD(0.8), FloatD(0.8)]
...
def uncertainty_f(sig_t_inner, ad=True):
    # Multi-group p-box sensitivity on the torus geometry using load_exp1_torus_openmc_multi_group.
    # Uncertain parameters: 3-group total cross sections for the INNER region.
    # Output y: middle energy channel (index 1) of Etot.

    func_name = "torus_multi"
    num_netrons = 100000

    # Keep geometry fixed for this sensitivity study
    geo_param = FloatD(0.0)

    # Accept either a length-3 iterable (preferred) or a scalar (legacy fallback)
    if isinstance(sig_t_inner, (list, tuple, np.ndarray)):
        if len(sig_t_inner) != 3:
            raise ValueError("sig_t_inner must have length 3 for multi-group sensitivity.")
        p0 = FloatD(sig_t_inner[0])
        p1 = FloatD(sig_t_inner[1])
        p2 = FloatD(sig_t_inner[2])
        if ad:
            dr.enable_grad(p0)
            dr.enable_grad(p1)
            dr.enable_grad(p2)
        sig_t_param = [p0, p1, p2]
    else:
        p0 = FloatD(sig_t_inner)
        if ad:
            dr.enable_grad(p0)
        # Legacy: only first group is a parameter
        sig_t_param = [p0, FloatD(0.8), FloatD(0.8)]
    ...
    if ad:
        ...
        val = float(y.numpy())
        if isinstance(sig_t_inner, (list, tuple, np.ndarray)):
            grad = np.array([dr.grad(p0).numpy(), dr.grad(p1).numpy(), dr.grad(p2).numpy()], dtype=float)
        else:
            grad = float(dr.grad(p0).numpy())
        del Etot, y
    else:
        val = float(y.numpy())
        if isinstance(sig_t_inner, (list, tuple, np.ndarray)):
