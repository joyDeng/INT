...
class Beams():
    ...
    def intersect3D(self, bl, tr, printout=False):
        ...
        exit_step_x = dr.select( dir.x < 0.0, dr.select(txl_valid, -1, 0), dr.select((dir.x > 0.0) & txr_valid, 1, 0))
        exit_step_y = dr.select( dir.y < 0.0, dr.select(tyb_valid, -1, 0), dr.select((dir.y > 0.0) & tyt_valid, 1, 0))
        exit_step_z = dr.select( dir.z < 0.0, dr.select(tzb_valid, -1, 0), dr.select((dir.z > 0.0) & tzf_valid, 1, 0))
