...
# =========================
RESULT_DIR     = "results_spheres_voxel"
SEED           = 12345
...
PARTICLES      = 100_000
BATCHES        = 50
MAX_BOUNCE     = 5                  # K (max scatters allowed)
...
# -------------------------
# Mesh (voxel) to match Mitsuba
# -------------------------
# We default to a cube enclosing the vacuum boundary sphere.
MESH_LOWER_LEFT  = (-R_BOUND, -R_BOUND, -R_BOUND)
MESH_UPPER_RIGHT = ( R_BOUND,  R_BOUND,  R_BOUND)
MESH_DIM         = (64, 64, 64)   # adjust to your Mitsuba voxel resolution
...
# ---------- Mesh collapse helpers (voxelized) ----------
def _mesh_energy_counts(T):
    Nvox, Gtot = None, None
    for f in T.filters:
        if f.__class__.__name__ == "MeshFilter":
            try:
                m = f.mesh
                Nvox = int(np.prod(m.dimension))
            except Exception:
                try:
                    Nvox = len(f.bins)
                except Exception:
                    pass
        if f.__class__.__name__ == "EnergyFilter":
            Gtot = int(len(_energy_bins_to_edges(f.bins)) - 1)
    return Nvox, Gtot

def collapse_mesh_to_g_and_stage(T, E_edges_master: np.ndarray, G: int, K: int):
    """Return (per_g, per_stage) arrays of shape (Nvox, G) and (Nvox, K+1)."""
    M = np.asarray(T.mean)
    # Determine expected sizes
    Nvox, Gtot = _mesh_energy_counts(T)
    if Nvox is None or Gtot is None:
        raise RuntimeError("Unable to infer mesh/energy counts from tally filters.")
    # Squeeze any trailing singleton dims and reshape
    M = M.reshape(-1)
    if M.size != Nvox * Gtot:
        # T.mean may be shape (Nvox, Gtot, 1[,1])
        try:
            M = np.asarray(T.mean)
            while M.ndim > 2:
                M = M[..., 0]
            if M.ndim == 2 and M.shape[0] * M.shape[1] == Nvox * Gtot:
                M = M.reshape(Nvox * Gtot)
            else:
                raise ValueError
        except Exception:
            raise RuntimeError(f"Unexpected tally mean shape for mesh collapse: {T.mean.shape}")
    M = M.reshape(Nvox, Gtot)

    bin_pairs = _pairs_from_energy_filter(T)
    if bin_pairs is None:
        raise RuntimeError("EnergyFilter bins not found in tally for mesh collapse.")
    if bin_pairs.shape[0] != Gtot:
        raise RuntimeError("EnergyFilter bin count does not match inferred Gtot.")

    pair_map = _build_pair_to_gk_map(E_edges_master, G, K)

    per_g     = np.zeros((Nvox, G), dtype=float)
    per_stage = np.zeros((Nvox, K+1), dtype=float)
    ...
    missing = 0
    for j, (lo, hi) in enumerate(bin_pairs):
        gk = pair_map.get(key(lo, hi))
        if gk is None:
            missing += 1
            continue
        g, k = gk
        ...
        per_stage[:, k] += M[:, j]
    if missing:
        print(f"[warn] {missing} mesh energy bins could not be mapped to (g,k); ignored.")
    return per_g, per_stage
...
def run_openmc_case(seed: int, outdir: str):
    ...
    mgxs_name = "spheres_voxel_mg_openmc_mgxs.h5"
    xs_abs, E_edges = build_mgxs_with_bounce_cap(MAX_BOUNCE, out_path=os.path.join(outdir, mgxs_name))
    ...
    # -------- Mesh (voxel) tallies to match Mitsuba ----------
    mesh = openmc.RegularMesh()
    mesh.lower_left  = tuple(map(float, MESH_LOWER_LEFT))
    mesh.upper_right = tuple(map(float, MESH_UPPER_RIGHT))
    mesh.dimension   = tuple(map(int, MESH_DIM))
    mf = openmc.MeshFilter(mesh)

    T_mesh_flux = openmc.Tally(name="Mesh_flux");              T_mesh_flux.scores = ["flux"];        T_mesh_flux.filters = [mf, ef]
    T_mesh_total = openmc.Tally(name="Mesh_total");            T_mesh_total.scores = ["total"];      T_mesh_total.filters = [mf, ef]
    T_mesh_abs = openmc.Tally(name="Mesh_absorption");         T_mesh_abs.scores  = ["absorption"]; T_mesh_abs.filters   = [mf, ef]
    T_mesh_scatA = openmc.Tally(name="Mesh_scatter_analog");   T_mesh_scatA.scores = ["scatter"];   T_mesh_scatA.filters = [mf, ef]; T_mesh_scatA.estimator = "analog"

    tallies = openmc.Tallies([
        T_flux_total, T_total_rate, T_rr_absorb, T_flux_by_cell, T_scatter_analog,
        T_mesh_flux, T_mesh_total, T_mesh_abs, T_mesh_scatA
    ])
    model = openmc.Model(materials=mats, geometry=geom, settings=sets, tallies=tallies)

    # ---------- Export & Run ----------
    run_dir = os.path.join(outdir, f"run_voxel_seed{seed}")
    ...
    print(f"[mesh] ll={mesh.lower_left}, ur={mesh.upper_right}, dim={mesh.dimension}")
    ...
    # ---- Mesh (voxel) collapse to (g) and (stage) ---
    T_mflux  = _discover_tally(sp, "Mesh_flux",           want_edges=E_edges, want_score="flux")
    T_mtotal = _discover_tally(sp, "Mesh_total",          want_edges=E_edges, want_score="total")
    T_mabs   = _discover_tally(sp, "Mesh_absorption",     want_edges=E_edges, want_score="absorption")
    T_mscatA = _discover_tally(sp, "Mesh_scatter_analog", want_edges=E_edges, want_score="scatter")

    vox_flux_g,  vox_flux_stage  = collapse_mesh_to_g_and_stage(T_mflux,  E_edges, G=3, K=K_infer)
    vox_tot_g,   vox_tot_stage   = collapse_mesh_to_g_and_stage(T_mtotal, E_edges, G=3, K=K_infer)
    vox_abs_g,   vox_abs_stage   = collapse_mesh_to_g_and_stage(T_mabs,   E_edges, G=3, K=K_infer)
    vox_scat_g   = np.clip(vox_tot_g  - vox_abs_g, 0.0, None)            # σs Φ per group (track-length)
    vox_scat_stg = np.clip(vox_tot_stage - vox_abs_stage, 0.0, None)     # σs Φ per stage
    vox_scatA_g, vox_scatA_stage = collapse_mesh_to_g_and_stage(T_mscatA, E_edges, G=3, K=K_infer)

    # Save voxel fields & metadata for side-by-side with Mitsuba
    Nvox = int(np.prod(MESH_DIM))
    save_npz = os.path.join(outdir, f"voxels_K{K_infer}_seed{seed}.npz")
    np.savez(
        save_npz,
        # metadata
        E_real_ASC=E_REAL_ASC,
        E_edges_expanded=E_edges,
        mesh_ll=np.array(MESH_LOWER_LEFT, float),
        mesh_ur=np.array(MESH_UPPER_RIGHT, float),
        mesh_dim=np.array(MESH_DIM, int),
        # voxelized fields (Nvox×G) and (Nvox×(K+1))
        flux_g=vox_flux_g,
        flux_stage=vox_flux_stage,
        sigma_s_phi_g=vox_scat_g,
        sigma_s_phi_stage=vox_scat_stg,
        scatter_analog_g=vox_scatA_g,
        scatter_analog_stage=vox_scatA_stage
    )
    meta_json = {
        "script": "spheres_voxel_mg_openmc.py",
        "seed": int(seed),
        "K_stages": int(K_infer+1),
        "groups_ASC": E_REAL_ASC.tolist(),
        "mesh": {
            "lower_left": list(map(float, MESH_LOWER_LEFT)),
            "upper_right": list(map(float, MESH_UPPER_RIGHT)),
            "dimension": list(map(int, MESH_DIM)),
            "Nvox": int(Nvox)
        },
        "mgxs_path": xs_abs,
        "run_dir": run_dir
    }
    with open(os.path.join(outdir, f"voxels_K{K_infer}_seed{seed}.json"), "w") as f:
        json.dump(meta_json, f, indent=2)
    print(f"[save] Voxel fields saved to: {save_npz}")
    print(f"[save] Meta saved to: {os.path.join(outdir, f'vexels_K{K_infer}_seed{seed}.json')}")
