...
| [area_tally.py](area_tally.py) | Main differentiable neutron transport simulation (track-length/collision tallies, energy-dependent transport). **Active development target** — supersedes `simulator.py`. |
| [simulator.py](simulator.py) | Earlier, stable version of the transport core. Frozen; kept only because `opt_sensor.py` still imports from it. |
...
The `*_june.py` files (`csg_june.py`, `area_tally_june.py`, `tests_june.py`) are a frozen October/November 2025 snapshot, superseded by the files above — not used by any current experiment.

---

## 1. Validation against OpenMC (multi-group)
...
## 2. Sensitivity analysis (p-box / uncertainty quantification)
...
## 4. Shape optimization without CSG (probability-based, `optimization.py`)
